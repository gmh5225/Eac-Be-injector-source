...
bool Inject::inject_module_from_path_to_process_by_name(const wchar_t* module_path, const wchar_t* process_name)
{
	...
	Driver* driver = new Driver(xor_w(L"\\\\.\\winmadsvc"), target_process_id);
	...
	uintptr_t shellcode_value = driver->allocate_virtual_memory(sizeof(int), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	printf(xor_a("Allocated 0x%p at %p\n"), 0x1000, allocated_shellcode);


	uintptr_t allocatedbase_offset = uintptr_t((uintptr_t)utils::find_pattern(xor_a("\x68\x41\x25\x46\x58\x01\x00\x00"), xor_a("xxxxxx??")) - (uintptr_t)&shellcode); 
	uintptr_t allocatedvalue_offset = uintptr_t((uintptr_t)utils::find_pattern(xor_a("\x53\x12\x84\x56\x48\x02\x00\x00"), xor_a("xxxxxx??")) - (uintptr_t)&shellcode);
